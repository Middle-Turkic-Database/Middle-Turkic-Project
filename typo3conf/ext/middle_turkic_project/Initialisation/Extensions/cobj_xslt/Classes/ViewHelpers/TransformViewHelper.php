    ...
    public function render()
    {
        ...
        if (count($transformations) > 0 && array_key_exists('stylesheet', $transformations[0])) {
            ...
            foreach ($transformations as $key => $transformation) {
                ...
                if (array_key_exists('setParameters', $transformation)) {
                    foreach ($transformation['setParameters'] as $parameterKey => $parameter) {
                        $configuration['transformations.'][$i]['setParameters.'][$parameterKey . '.']['value'] = $parameter['value'];
                    }
                }
            }
